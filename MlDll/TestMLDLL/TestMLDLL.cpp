...
int main()
{
    ...
    std::cout << "Weights : " << model[0] << ", " << model[1] << ", " << model[2] << std::endl;
    

    std::cout << "BEFORE !" << std::endl;
    for (size_t i = 0; i < test.sample_count; i ++)
    {
        std::cout << predict_linear_model_classification(model, new double[2]{ test.samples[i], test.samples[i + 1] }, 2) << std::endl;
    }
